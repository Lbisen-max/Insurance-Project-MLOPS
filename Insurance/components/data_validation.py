# Here checking data type
# Unwanted data fining 
# data cleaning
...
class DataValidation:
    ...
    def drop_missing_value_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        try:
            ...
            # if there is no missing value then run blow code
            if len(df.columns)==0:
                return None
            return df
        ...
        try:
            ...
            for base_column in base_columns:
                if base_column not in current_columns:
                    logging.info(f"column:[{base_column} is not availanle]")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df.replace({"np":np.NAN},inplace=True)
            ...
            train_df_columns_status = self.is_required_column_exists(base_df=base_df,current_df=train_df,report_key_name="Missing_values_within_train_dataset")
            test_df_columns_status = self.is_required_column_exists(base_df=base_df,current_df=test_df,report_key_name="Missing_values_within_test_dataset")
